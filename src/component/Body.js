...
const Body = () => {
  ...
  if (!onlineStatus) {
    return (
      <h1>Sorry ! Something is wrong . Check yout internet connection.</h1>
    );
  }
  ...
  const FilterBar = () => {
    return (
      <div className="search-container">
        ...
        <label className="top-rated-rest-btn">
          Username :
        </label>
        <input
          type="text"
    ...
    <div className="body">
      ...
      <div className="restaurant-list">
        {filteredRest.map((restaurant) => (
          <RestaurantCards key={restaurant.info.id} {...restaurant.info} />
        ))}
      </div>
    </div>
  );
